...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Run the Shortage Scout pipeline (acquire → transform → output)"
    )
    parser.add_argument(
        "--start-date",
        dest="start_date",
        help="Start date for acquisition (YYYY-MM-DD)",
        default=None,
    )
    parser.add_argument(
        "--end-date",
        dest="end_date",
        help="End date for acquisition (YYYY-MM-DD)",
        default=None,
    )
    parser.add_argument(
        "--demo",
        dest="demo",
        action="store_true",
        help="Force demo mode using synthetic data",
    )
