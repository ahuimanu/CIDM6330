    ...
    for attempt in range(max_retries):
        try:
            ...
            if 500 <= response.status_code < 600:
                ...
                print(
                    f"Attempt {attempt + 1} failed with {response.status_code}. Retrying in {wait_time} seconds..."
                )
...
def main():
    ...
    if response and response.status_code == 200:
        ...
        # Save to JSON file (relative to this module)
        out_path = os.path.join(
            os.path.dirname(__file__), "..", "data", "unrate_data.json"
        )
