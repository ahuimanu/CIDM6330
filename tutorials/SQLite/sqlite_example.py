...
This module demonstrates Python's sqlite3 capabilities using
a weather reporting domain model with airports, runways, and weather data.
...
@dataclass
class Airport:
    """Airport with optional runways and weather."""
    id: int | None
    stationid: str
    name: str
    city: str
...
class AirportRepository(ABC):
    """Abstract base class defining airport data access interface."""
...
class SQLiteAirportRepository(AirportRepository):
    """SQLite implementation of airport repository."""
    ...
    def save(self, airport: Airport) -> Airport:
        if airport.id is None:
            ...
            self._conn.execute("""
                ...
                WHERE id = ?
            """, (airport.stationid, airport.name, airport.city, airport.state,
                  airport.latitude, airport.longitude, airport.id))
    ...
    def _save_runway(self, runway: Runway) -> Runway:
        cursor = self._conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (runway.airport_id, runway.identifier, runway.length_ft,
              runway.width_ft, runway.surface))
    ...
    def _save_weather(self, weather: WeatherReport) -> WeatherReport:
        cursor = self._conn.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (weather.airport_id, weather.metar, weather.taf,
              weather.fetched_at.isoformat()))
        weather.id = cursor.lastrowid
        return weather
    
    def _row_to_airport(self, row: sqlite3.Row, include_relations: bool = False) -> Airport:
        airport = Airport(
            id=row["id"],
...
class AirportQuery:
    """Fluent query builder for airports."""
...
def main() -> None:
    ...
    # UPDATE
    conn.execute("UPDATE airports SET name = ? WHERE stationid = ?",
    ...
    with UnitOfWork(":memory:") as uow:
        ...
        # Add more airports
        uow.airports.save(Airport(None, "KHOU", "William P. Hobby Airport", 
                                   "Houston", "TX"))
        uow.airports.save(Airport(None, "KSAT", "San Antonio International", 
                                   "San Antonio", "TX"))
