...
class Airport:
    ...
    def from_dict(cls, data: dict) -> "Airport":
        ...
        weather = WeatherReport.from_dict(weather_data) if weather_data else WeatherReport()
...
def save_airports_csv(airports: list[Airport], path: Path) -> None:
    ...
    fieldnames = ["stationid", "name", "city", "state", "runway_count", "longest_runway_ft", "metar"]
...
class AppConfig:
    ...
    log_level: str = "INFO"
    enabled_stations: list[str] = field(default_factory=lambda: ["KAMA", "KLBB", "KMAF"])
...
def main() -> None:
    ...
    with temporary_workspace() as workspace:
        ...
        print(f"  Original:   {original.stationid} with {len(original.runways)} runways")
        print(f"  From JSON:  {from_json.stationid} with {len(from_json.runways)} runways")
        print(f"  Match: {original.stationid == from_json.stationid}")
        print(f"  Runway match: {original.runways[0].length_ft == from_json.runways[0].length_ft}")
