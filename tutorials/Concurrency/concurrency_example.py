...
def executor_error_handling_example() -> None:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(flaky_fetch, a.stationid): a for a in AIRPORTS[:4]}
...
async def asyncio_basic_example() -> None:
    ...
    reports = await asyncio.gather(
        *[simulate_io_bound_async(a.stationid) for a in AIRPORTS]
    )
...
async def asyncio_tasks_example() -> None:
    ...
    tasks = [asyncio.create_task(fetch_with_logging(a.stationid)) for a in AIRPORTS[:3]]
...
async def asyncio_semaphore_example() -> None:
    ...
    start = time.perf_counter()
    results = await asyncio.gather(*[limited_fetch(a.stationid) for a in AIRPORTS])
    elapsed = time.perf_counter() - start
...
def compare_approaches() -> None:
    ...
    # Asyncio
    async def run_async():
        await asyncio.gather(*[simulate_io_bound_async(sid) for sid in stationids])
